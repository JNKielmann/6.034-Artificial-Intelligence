...
daughter = IF(AND('parent (?x) (?y)', 'female (?y)'), THEN('daughter (?y) (?x)'))
cousin = IF(AND('sibling (?x) (?a)', 'parent (?x) (?y)', 'parent (?a) (?b)'), THEN('cousin (?y) (?b)'))
...
# family_rules.
family_rules = [ same_identity, sibling, brother, sister, father, mother, son, daughter, cousin, grandparent, grandchild ]                    # fill me in
